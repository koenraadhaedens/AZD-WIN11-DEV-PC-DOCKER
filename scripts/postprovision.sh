...
# comment the lines below if you do not want to send stats to a table
 ...
 deploymentData=$(jq -n --arg deployment "tdd-azd-devpc" --arg location "$location" --arg environmentName "$environmentName" --arg machine "$AZUREPS_HOST_ENVIRONMENT" --arg commitHash "$commitHash" '{Deployment: $deployment, location: $location, environmentName: $environmentName, Machine: $machine, CommitHash: $commitHash}')
